...
let
  ...
  cfg = config.services.hexstody-eth;
in {
  ##### interface. here we define the options that users of our service can specify
  options = {
    # the options for our service will be located under services.hexstody-btc
    services.hexstody-eth = {
      enable = mkOption {
        type = types.bool;
        default = false;
        description = ''
          Whether to enable hexstody ETH adapter service by default.
        '';
      };
      package = mkOption {
        type = types.package;
      ...
      port = mkOption {
        ...
        default = 8540;
        description = ''
          Which port the ETH adapter listen to serve API.
        '';
      };
      host = mkOption {
        type = types.str;
